...
def calcular_residuos(dados_estimados, dados_referencia_comercial):
    """
    Calcula los residuos como la diferencia entre la última columna de 
    dados_referencia_comercial y dados_estimados. Luego devuelve un array
    con todas las columnas originales de dados_estimados más esta nueva columna
    de residuos.

    Parámetros:
    - dados_estimados: np.ndarray
        Array con las predicciones o estimaciones.
    - dados_referencia_comercial: np.ndarray
        Array con los valores reales o de referencia.

    Retorna:
    - np.ndarray:
        Array con los mismos datos de entrada sin la última columna,
        más una nueva columna con los residuos calculados.
    """
    # Calcular residuos
    residuos = dados_referencia_comercial[:, -1] - dados_estimados[:, -1]
    
    # Redimensionar residuos a columna
    residuos_reshape = residuos.reshape(-1, 1)
    
    # Concatenar todas las columnas excepto la última con los residuos
    resultado = np.hstack((dados_estimados[:, :-1], residuos_reshape))
...
def ML_model_evaluation(dados_simulacao=None, proporcao_treino=0.75, modelo="extratrees"):
    ...
    return sim_estimado, y, nrms_teste, r2_teste, mape_teste, modelo, dict_params, ET
